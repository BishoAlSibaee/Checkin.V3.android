...
public class Suite extends Bed {
    ...
    public static List<CheckinDevice> SuitesDevices = new ArrayList<>();
    ...
    public void setFireRoom(FirebaseDatabase database) {
        fireSuite = database.getReference(MyApp.My_PROJECT.projectName+"/B"+building.buildingNo+"/F"+floor.floorNumber+"/S"+SuiteNumber);
        devicesControlReference = database.getReference(MyApp.My_PROJECT.projectName+"Devices/"+SuiteNumber);
        devicesDataReference = database.getReference(MyApp.My_PROJECT.projectName+"DevicesData/"+SuiteNumber);
...
//        });
    ...
    void setMyRooms() {
        if (RoomsId != null) {
            ...
            for (String s : roomsIdsArray) {
                for (int j = 0; j < MyApp.ROOMS.size(); j++) {
                    if (Integer.parseInt(s) == MyApp.ROOMS.get(j).id) {
                        RoomsList.add(MyApp.ROOMS.get(j));
                    }
                }
            }
        }
    ...
    public static void setSuitesFireSuites(List<Suite> suites,FirebaseDatabase database) {
        for (Suite s :suites) {
            s.setFireRoom(database);
        }
    }
