...
import com.syriasoft.server.Classes.Devices.CheckinAC;
import com.syriasoft.server.Classes.Devices.CheckinCurtain;
import com.syriasoft.server.Classes.Devices.CheckinDevice;
import com.syriasoft.server.Classes.Devices.CheckinDoorSensor;
import com.syriasoft.server.Classes.Devices.CheckinGateway;
import com.syriasoft.server.Classes.Devices.CheckinLock;
import com.syriasoft.server.Classes.Devices.CheckinMotionSensor;
import com.syriasoft.server.Classes.Devices.CheckinPower;
import com.syriasoft.server.Classes.Devices.CheckinServiceSwitch;
import com.syriasoft.server.Classes.Devices.CheckinShutter;
import com.syriasoft.server.Classes.Devices.CheckinSwitch;
import com.syriasoft.server.Classes.Enumes.DeviceTypes;
import com.syriasoft.server.Classes.Interfaces.ACListener;
import com.syriasoft.server.Classes.Interfaces.CurtainListener;
...
import com.syriasoft.server.Classes.Interfaces.LockListener;
...
public class Room extends Bed {
    ...
    ValueEventListener roomStatusListener,clientInListener,firstDoorOpenListener;

    public boolean byLink = false; // from firebase
    public String token ; // from server
    Building building;
    Floor floor;
    public List<CheckinSwitch> switches;
    ...
    String FirstRunDevices = "firstRunDevices";
    Timer closeCurtainTimer;
    ...
    public void setFireRoomListeners() {
        ...
        roomStatusListener = fireRoom.child("roomStatus").addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.getValue() != null ) {
                    if (firstRun[0]) {
                        Log.d("checkinMood"+RoomNumber,"first run "+ firstDoorOpen);
                    }
                            ...
                            if (MyApp.My_PROJECT.projectName.equals("kfu")) {
                                if (isHasAC()) {
                                    for (CheckinAC ac : acs) {
                                        ac.turnOff(new IResultCallback() {
                                            @Override
        ...
        firstDoorOpenListener = fireRoom.child("firstDoorOpen").addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.getValue() != null) {
                    firstDoorOpen = Boolean.parseBoolean(snapshot.getValue().toString());
                    Log.d("checkinMood"+RoomNumber,"first door open value "+ firstDoorOpen);
                }
            }
    ...
    public void removeFireRoomListeners() {
        ...
        if (firstDoorOpenListener != null) {
            fireRoom.child("firstDoorOpen").removeEventListener(firstDoorOpenListener);
        }
    }
    ...
    public List<CheckinDevice> setMyDevices(List<DeviceBean> devices,HomeBean homeBean) {
        ...
        for (int i=0;i<devices.size();i++) {
            ...
            else if (d.getName().contains(RoomNumber+DeviceTypes.Shutter.toString())) {
                if (shutters == null) {
                    shutters = new ArrayList<>();
                }
                CheckinShutter shu = new CheckinShutter(d,this);
                shutters.add(shu);
                devs.add(shu);
            }
        }
        ...
        if (isHasServiceSwitch()) {
            ...
            getMainServiceSwitch().listen(new ServiceListener() {
                ...
                public void cleanup() {
                    if (first[0]) {
                        first[0] = false;
                        Log.d(FirstRunDevices,RoomNumber+" cleanup on");
                    }
                    Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                    if (roomStatus == 2) {
                ...
                public void cancelCleanup() {
                    if (first[0]) {
                        first[0] = false;
                        Log.d(FirstRunDevices,RoomNumber+" cleanup off");
                    }
                    Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                    if (roomStatus == 2) {
                ...
                public void laundry() {
                    if (first[0]) {
                        first[0] = false;
                        Log.d(FirstRunDevices,RoomNumber+" laundry on");
                    }
                    Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                    if (roomStatus == 2) {
                ...
                public void cancelLaundry() {
                    if (first[0]) {
                        first[0] = false;
                        Log.d(FirstRunDevices,RoomNumber+" laundry off");
                    }
                    Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                    if (roomStatus == 2) {
                ...
                public void dnd() {
                    if (first[0]) {
                        first[0] = false;
                        Log.d(FirstRunDevices,RoomNumber+" dnd on");
                    }
                    Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                    if (roomStatus == 2) {
                        addDndOrder();
                ...
                public void cancelDnd() {
                    if (first[0]) {
                        first[0] = false;
                        Log.d(FirstRunDevices,RoomNumber+" dnd off");
                    }
                    Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                    if (roomStatus == 2) {
                        cancelDndOrder();
                ...
                public void checkout() {
                    if (first[0]) {
                        first[0] = false;
                        Log.d(FirstRunDevices,RoomNumber+" checkout on");
                    }
                    Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                    if (roomStatus == 2) {
                ...
                @Override
                public void cancelCheckout() {if (first[0]) {
                    first[0] = false;
                    Log.d(FirstRunDevices,RoomNumber+" checkout off");
                }
                    Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                    if (roomStatus == 2) {
                ...
                public void lightOn() {
                    ...
                    Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                    devicesControlReference.child(getMainServiceSwitch().device.name).child(String.valueOf(getMainServiceSwitch().light.dpId)).setValue(3);
                    ControlDevice.setCurrentAction(tv,"light on room " + RoomNumber);
                }
                ...
                public void lightOff() {
                    ...
                    Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                    devicesControlReference.child(getMainServiceSwitch().device.name).child(String.valueOf(getMainServiceSwitch().light.dpId)).setValue(0);
                    ControlDevice.setCurrentAction(tv,"light off room " + RoomNumber);
                }
            });
        }
        if (isHasCurtain()) {
            ...
            for (CheckinSwitch cs : switches) {
                ...
                cs.listen(new SwitchListener() {
                    ...
                    public void oneOn() {
                        if (first[0]) {
                            first[0] = false;
                            Log.d(FirstRunDevices,RoomNumber+" "+cs.device.name+" 1 on");
                        }
                        Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                    ...
                    public void oneOff() {
                        if (first[0]) {
                            first[0] = false;
                            Log.d(FirstRunDevices,RoomNumber+" "+cs.device.name+" 1 off");
                        }
                        Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                    ...
                    public void secondOn() {
                        if (first[0]) {
                            first[0] = false;
                            Log.d(FirstRunDevices,RoomNumber+" "+cs.device.name+" 2 on");
                        }
                        Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                    ...
                    public void secondOff() {
                        if (first[0]) {
                            first[0] = false;
                            Log.d(FirstRunDevices,RoomNumber+" "+cs.device.name+" 2 off");
                        }
                        Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                    ...
                    public void thirdOn() {
                        if (first[0]) {
                            first[0] = false;
                            Log.d(FirstRunDevices,RoomNumber+" "+cs.device.name+" 3 on");
                        }
                        Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                    ...
                    public void thirdOff() {
                        if (first[0]) {
                            first[0] = false;
                            Log.d(FirstRunDevices,RoomNumber+" "+cs.device.name+" 3 off");
                        }
                        Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                    ...
                    public void forthOn() {
                        if (first[0]) {
                            first[0] = false;
                            Log.d(FirstRunDevices,RoomNumber+" "+cs.device.name+" 4 on");
                        }
                        Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                    ...
                    public void forthOff() {
                        if (first[0]) {
                            first[0] = false;
                            Log.d(FirstRunDevices,RoomNumber+" "+cs.device.name+" 4 off");
                        }
                        Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                    ...
                    public void online(boolean online) {
                        if (first[0]) {
                            first[0] = false;
                            Log.d(FirstRunDevices,RoomNumber+" "+cs.device.name+" online");
                        }
                        Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                        cs.online = online;
                    }
...
//                    }
                ...
                public void online(boolean online) {
                    Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                    getMainRoomDoorSensor().online = online;
                }
            });
        }
        if (isHasMotion()) {
...
//                        }
        ...
        if (isHasShutter()) {
            for (CheckinShutter cs : shutters) {
                cs.listen(new ShutterListener() {
                    @Override
                    public void openShutterOn() {
                        Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                        devicesControlReference.child(cs.device.name).child(String.valueOf(cs.dp1.dpId)).setValue(3);
                        ControlDevice.setCurrentAction(tv,"Room " + RoomNumber + " "+cs.device.name+ " open");
                    }

                    @Override
                    public void openShutterOff() {
                        Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                        devicesControlReference.child(cs.device.name).child(String.valueOf(cs.dp1.dpId)).setValue(0);
                    }

                    @Override
                    public void closeShutterOn() {
                        Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                        devicesControlReference.child(cs.device.name).child(String.valueOf(cs.dp2.dpId)).setValue(3);
                        ControlDevice.setCurrentAction(tv,"Room " + RoomNumber + " "+cs.device.name+ " close");
                    }

                    @Override
                    public void closeShutterOff() {
                        Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                        devicesControlReference.child(cs.device.name).child(String.valueOf(cs.dp2.dpId)).setValue(0);
                    }
    ...
    public void setRoomDevicesListener(RequestQueue CLEANUP_QUEUE,RequestQueue LAUNDRY_QUEUE,RequestQueue CHECKOUT_QUEUE) {
        Room r = this;
        if (isHasPower()) {
            ...
            power.listen(new PowerListener() {
                ...
                public void powerOn() {
                    ...
                    setRoomPowerStatus(2);
                    //ControlDevice.setCurrentAction(tv,"Room " + RoomNumber + " power on");
                    //ServerService.putAction("Room " + RoomNumber + " power on");
                }
                ...
                public void powerOff() {
                    ...
                    setRoomPowerStatus(0);
                    //ControlDevice.setCurrentAction(tv,"Room " + RoomNumber + " power off");
                    //ServerService.putAction("Room " + RoomNumber + " power off");
                }
                ...
                public void powerByCard() {
                    ...
                    Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                    setRoomPowerStatus(1);
                    //ServerService.putAction("Room " + RoomNumber + " power byCard");
                }
        ...
        if (isHasServiceSwitch()) {
            getMainServiceSwitch().listen(new ServiceListener() {
                ...
                public void cleanup() {
                    ...
                    //ServerService.putAction("Cleanup on room " + RoomNumber);
                }
                ...
                public void cancelCleanup() {
                    ...
                    //ServerService.putAction("Cleanup canceled room " + RoomNumber);
                }
                ...
                public void laundry() {
                    ...
                    //ServerService.putAction("Laundry on room " + RoomNumber);
                }
                ...
                public void cancelLaundry() {
                    ...
                    //ServerService.putAction("Laundry canceled room " + RoomNumber);
                }
                ...
                public void dnd() {
                    ...
                    //ServerService.putAction("DND on room " + RoomNumber);
                }
                ...
                public void cancelDnd() {
                    ...
                    //ServerService.putAction("DND canceled room " + RoomNumber);
                }
                ...
                public void checkout() {
                    ...
                    //ServerService.putAction("Checkout on room " + RoomNumber);
                }
                ...
                public void cancelCheckout() {
                    ...
                    //ServerService.putAction("Checkout canceled room " + RoomNumber);
                }
            ...
            for (CheckinCurtain cc:curtains) {
                cc.listen(new CurtainListener() {
                    ...
                    public void open() {
                        Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                        setRoomCurtainStatus(1);
                        //ServerService.putAction("Curtain open room " + RoomNumber);
                    }
                    ...
                    public void close() {
                        Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                        setRoomCurtainStatus(0);
                        //ServerService.putAction("Curtain close room " + RoomNumber);
                    }
            ...
            for (CheckinSwitch cs : switches) {
                cs.listen(new SwitchListener() {
                    ...
                    public void oneOn() {
                        Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                        devicesControlReference.child(cs.device.name).child(String.valueOf(cs.dp1.dpId)).setValue(3);
                        //ServerService.putAction("Room " + RoomNumber + " "+cs.device.name+ " button "+cs.dp1.dpId+" on");
                    }
                    ...
                    public void oneOff() {
                        Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                        devicesControlReference.child(cs.device.name).child(String.valueOf(cs.dp1.dpId)).setValue(0);
                        //ServerService.putAction("Room " + RoomNumber + " "+cs.device.name+ " button "+cs.dp1.dpId+" off");
                    }
                    ...
                    public void secondOn() {
                        Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                        devicesControlReference.child(cs.device.name).child(String.valueOf(cs.dp2.dpId)).setValue(3);
                        //ServerService.putAction("Room " + RoomNumber + " "+cs.device.name+ " button "+cs.dp2.dpId+" on");
                    }
                    ...
                    public void secondOff() {
                        Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                        devicesControlReference.child(cs.device.name).child(String.valueOf(cs.dp2.dpId)).setValue(0);
                        //ServerService.putAction("Room " + RoomNumber + " "+cs.device.name+ " button "+cs.dp2.dpId+" off");
                    }
                    ...
                    public void thirdOn() {
                        Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                        devicesControlReference.child(cs.device.name).child(String.valueOf(cs.dp3.dpId)).setValue(3);
                        //ServerService.putAction("Room " + RoomNumber + " "+cs.device.name+ " button "+cs.dp3.dpId+" on");
                    }
                    ...
                    public void thirdOff() {
                        Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                        devicesControlReference.child(cs.device.name).child(String.valueOf(cs.dp3.dpId)).setValue(0);
                        //ServerService.putAction("Room " + RoomNumber + " "+cs.device.name+ " button "+cs.dp3.dpId+" off");
                    }
                    ...
                    public void forthOn() {
                        Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                        devicesControlReference.child(cs.device.name).child(String.valueOf(cs.dp4.dpId)).setValue(3);
                        //ServerService.putAction("Room " + RoomNumber + " "+cs.device.name+ " button "+cs.dp4.dpId+" on");
                    }
                    ...
                    public void forthOff() {
                        Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                        devicesControlReference.child(cs.device.name).child(String.valueOf(cs.dp4.dpId)).setValue(0);
                        //ServerService.putAction("Room " + RoomNumber + " "+cs.device.name+ " button "+cs.dp4.dpId+" off");
                    }
        ...
        if (isHasLock()) {
            getRoomLock().listen(new LockListener() {
                ...
                public void unlocked() {
                    Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                    devicesControlReference.child(getRoomLock().device.name).child("1").setValue(0);
                    //ServerService.putAction("Room " + RoomNumber + " lock unlocked");
                }
                ...
                public void battery(int battery) {
                    Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                    fireRoom.child("lockBattery").setValue(battery);
                    //ServerService.putAction("Room " + RoomNumber + " lock battery "+battery);
                }
            ...
            for (CheckinAC ca : acs) {
                ca.listen(new ACListener() {
                    ...
                    public void onPowerOn() {
                        Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                        devicesControlReference.child(ca.name).child("power").setValue(3);
                        //ServerService.putAction("Room " + RoomNumber + " ac turn on");
                    }
                    ...
                    public void onPowerOff() {
                        Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                        devicesControlReference.child(ca.name).child("power").setValue(0);
                        //ServerService.putAction("Room " + RoomNumber + " ac turn off");
                    }
                    ...
                    public void onTempSet(String newTemp) {
                        ...
                        //ServerService.putAction("Room " + RoomNumber + " ac set to "+newTemp);
                    }
                    ...
                    public void onFanSet(String newFan) {
                        Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                        //ServerService.putAction("Room " + RoomNumber + " ac fan set to "+newFan);
                    }
        ...
        if (isHasDoorSensor()) {
            getMainRoomDoorSensor().listen(new DoorListener() {
                ...
                public void open() {
                    ...
                    setRoomDoorStatus(1);
                    if (roomStatus == 2) {
                        Log.d("checkinMood"+RoomNumber,"room booked");
                        if (firstDoorOpen) {
                            Log.d("checkinMood"+RoomNumber,"first open");
                            Log.d("checkinMood"+RoomNumber,"room nut null");
                            //PROJECT_VARIABLES.checkInMood.startCheckinMood(r);
                            if (MyApp.My_PROJECT.projectName.equals("P0003")) {
                                PROJECT_VARIABLES.checkInMood.startCheckinMood(r);
//                                turnLightsOn();
...
//                        if (isHasPower()) {
//                            if (getPowerModule().dp1.getCurrent() && getPowerModule().dp2.getCurrent()) {
//                                turnLightsOn();
//                            }
//                            else if (!getPowerModule().dp2.getCurrent()) {
//                                powerOnRoom(new IResultCallback() {
//                                    @Override
//                                    public void onError(String code, String error) {
//
...
//                                });
//                                turnLightsOn();
//                                powerByCardAfterMinutes(5, new RequestCallback() {
//                                    @Override
//                                    public void onSuccess() {
//
//                                    }
...
//                        }
                    ...
                    //ServerService.putAction("Room " + RoomNumber + " door open");
                }
                ...
                public void close() {
                    ...
                    if (MyApp.My_PROJECT.projectName.equals("apiTest")) {
                        ...
                        // for ac scenario
                        if (RoomNumber == 210 || RoomNumber == 1040 || RoomNumber == 211) {
                            if (PROJECT_VARIABLES.isAcScenarioActive()) {
                                if (doorWarningTimer != null) {
                                    doorWarningTimer.cancel();
                                    doorWarningTimer = null;
                    ...
                    //ServerService.putAction("Room " + RoomNumber + " door close");
                }

                @Override
                public void battery(int battery) {
                    Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                    //ServerService.putAction("Room " + RoomNumber + " door sensor battery "+battery);
                }
...
//                        }
                        // for ac scenario
                        if (MyApp.My_PROJECT.projectName.equals("apiTest")) {
                            if (RoomNumber == 210 || RoomNumber == 104 || RoomNumber == 211) {
                                if (acScenarioStarted) {
                                    setAcToSetPoint();
                                    Log.d("acScenario", RoomNumber + " setpoint done " + PROJECT_VARIABLES.Temp);
                                }
    ...
    private void setFirstDoorOpen(boolean status) {
        firstDoorOpen = status;
        fireRoom.child("firstDoorOpen").setValue(firstDoorOpen);
    }
    ...
    public boolean isHasShutter() {
        return shutters != null && !shutters.isEmpty();
    }
    ...
    public void powerOnAfterMinutes(int minutes, RequestCallback callback) {
        if (power != null) {
            Timer t = new Timer();
            t.schedule(new TimerTask() {
                @Override
    ...
    public void powerByCardAfterMinutes(int minutes, RequestCallback callback) {
        if (power != null) {
            Log.d("checkinMood"+RoomNumber,"power card set after "+minutes+" minutes");
            Timer t = new Timer();
            t.schedule(new TimerTask() {
                @Override
                public void run() {
                    Log.d("checkinMood"+RoomNumber,"power card run");
                    powerByCardRoom(new IResultCallback() {
                        @Override
                        public void onSuccess() {
                            Log.d("checkinMood"+RoomNumber,"power card done");
                            callback.onSuccess();
                        }

                        @Override
                        public void onError(String code,String error) {
                            Log.d("checkinMood"+RoomNumber,"power card failed "+error);
                            callback.onFail(error);
                        }
                    });
                }
...
//    }
    ...
    public static void setRoomsBuildingsAndFloors(List<Room> rooms,List<Building> buildings,List<Floor> floors) {
        ...
        for (Floor f :floors) {
            Log.d("roomBuildingFloor",f.id+" "+f.floorNumber+" "+f.rooms);
        }
        for (Room r :rooms) {
    ...
    public static void setRoomsFireRoomsDevicesControlListener(List<Room> rooms) {
        for (Room room : rooms) {
            ...
            if (room.isHasShutter()) {
                for (CheckinShutter cs : room.shutters) {
                    cs.setFirebaseDevicesControl(room.devicesControlReference);
                }
            }
        }
    ...
    public static void setRoomsDevicesListener(List<Room> rooms,RequestQueue CQ,RequestQueue LQ,RequestQueue CHQ) {
        for (Room room:rooms) {
            Log.d("deviceListener"+room.RoomNumber,"started");
            room.setRoomDevicesListener(CQ,LQ,CHQ);
        }
    }
...
//        }
        ...
        if (MyApp.My_PROJECT.projectName.equals("apiTest")) {
            ...
            if (RoomNumber == 104) {
                if (isHasSwitch()) {
                    for (CheckinSwitch cs : switches) {
                        ...
                        if (cs.dp3 != null) {
                            cs.dp3.turnOn(new IResultCallback() {
                                @Override
                                public void onError(String code, String error) {
        ...
        else if (MyApp.My_PROJECT.projectName.equals("P0003")) {
            if (isHasSwitch()) {
                for (CheckinSwitch cs : switches) {
                    if (cs.device.name.contains("Switch3")) {
            ...
            if (getMainServiceSwitch() != null) {
                if (getMainServiceSwitch().light != null) {
                    getMainServiceSwitch().light.turnOn(new IResultCallback() {
                        @Override
                        public void onError(String code, String error) {
