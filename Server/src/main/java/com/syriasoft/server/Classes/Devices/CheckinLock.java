...
import com.syriasoft.server.Classes.Interfaces.RecordUnlock;
...
import com.syriasoft.server.Interface.RequestOrder;
import com.syriasoft.server.MyApp;
import com.syriasoft.server.Classes.ZigbeeLock;
...
public class CheckinLock extends CheckinDevice implements Listen, SetFirebaseDevicesControl, SetInitialValues {
    ...
    public void setInitialCurrentValues(RequestCallback callback) {
        if (my_room != null) {
            my_room.fireRoom.child("lock").setValue(1);
        }
        callback.onSuccess();
    }
