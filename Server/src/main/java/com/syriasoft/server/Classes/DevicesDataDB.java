...
import com.syriasoft.server.Classes.Devices.BoolKeyValue;
import com.syriasoft.server.Classes.Devices.CheckinDevice;
import com.syriasoft.server.Classes.Devices.DeviceDP;
import com.syriasoft.server.Classes.Devices.DeviceDPBool;
import com.syriasoft.server.Classes.Devices.DeviceDPEnum;
import com.syriasoft.server.Classes.Devices.DeviceDPValue;
import com.syriasoft.server.Classes.Devices.EnumKeyValue;
import com.syriasoft.server.Classes.Devices.ValueKeyValue;
...
public class DevicesDataDB extends SQLiteOpenHelper {
    ...
    void getDPAndWriteItToDevice(Cursor c,List<CheckinDevice> devices,RequestCallback callback) {
        boolean res = false;
        String deviceId = c.getString(1);
        CheckinDevice cd = CheckinDevice.getDeviceById(devices,deviceId);
        if (cd != null) {
        ...
        if (res) {
            ...
            callback.onFail(c.getString(2)+" dp "+c.getString(4)+" getting failed");
        }
    }
