...
public class ReceivingService extends Service {
    ...
    public void onCreate() {
        ...
        for (int i = 0; i < Rooms.size(); i++) {
            ...
            Rooms.get(i).setFireRoom(database.getReference(MyApp.MyProject.projectName + "/B" + Rooms.get(i).Building + "/F" + Rooms.get(i).Floor + "/R" + Rooms.get(i).RoomNumber));
            CLEANUP_RCODE[i] = 0;
            LAUNDRY_RCODE[i] = 0;
            DND_RCODE[i] = 0;
            SOS_RCODE[i] = 0;
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        if (Long.parseLong(dataSnapshot.getValue().toString()) > 0 && RESTAURANT_RCODE[finalI] == 0) {
                            Rooms.get(finalI).getFireRoom().child("Facility").addListenerForSingleValueEvent(new ValueEventListener() {
                                @Override
                                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                    if (Long.parseLong(dataSnapshot.getValue().toString()) == RestaurantOrders.THE_FACILITY.id) {
                                        int reqCode = r.nextInt();
