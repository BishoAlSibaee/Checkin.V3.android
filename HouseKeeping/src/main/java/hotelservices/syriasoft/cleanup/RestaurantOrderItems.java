...
public class RestaurantOrderItems extends AppCompatActivity {
    ...
    void setActivity() {
        ...
        FacilityName.setText(RestaurantOrders.THE_FACILITY.Name);
        ...
        Picasso.get().load(RestaurantOrders.THE_FACILITY.photo).into(FacilityImage);
    ...
    public static void getorderItems() {
        ...
        String url = MyApp.MyProject.Url + "facilitys/getRestOrderItems";
    ...
    public void maeOrderDone(View view) {
        LoadingDialog loadingDialog = new LoadingDialog(act);
        String url = MyApp.MyProject.Url + "facilitys/finishRestOrder";
            ...
            public void onResponse(String response) {
                ...
                try {
                    ...
                    if (result.getString("result").equals("success")) {
                        ...
                        DatabaseReference r = database.getReference(MyApp.MyProject.projectName+"/B"+RestaurantOrders.SELECTED_ROOM.Building+"/F"+RestaurantOrders.SELECTED_ROOM.Floor+"/R"+RestaurantOrders.SELECTED_ROOM.RoomNumber);
                        r.child("Restaurant").setValue(0);
                        r.child("Facility").setValue(0);
                        act.finish();
                    }
                    else {
                        new messageDialog(result.getString("error"),"error",act);
