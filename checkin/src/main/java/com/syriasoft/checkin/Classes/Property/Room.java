...
public class Room {
    ...
    public List<CheckinDevice> setMyDevices(List<DeviceBean> devices,HomeBean homeBean) {
        ...
        for (int i=0;i<devices.size();i++) {
            ...
            Log.d("serviceProb",RoomNumber+" "+d.name);
            if (d.getName().contains(DeviceTypes.Gateway.toString()+RoomNumber) || d.getName().contains(RoomNumber+"ZGatway") || d.getName().contains(RoomNumber+"Gatway")) {
            ...
            else if (d.getName().contains(RoomNumber+DeviceTypes.Power.toString())) {
                //Log.d("getDevicesRun","power found "+d.name);
                CheckinPower p = new CheckinPower(d,this);
                //Log.d("getDevicesRun","power casted "+p.device.name);
                power = p;
                devs.add(p);
            }
            else if (d.getName().contains(RoomNumber+DeviceTypes.Lock.toString())) {
                if (locks == null) {
                    locks = new ArrayList<>();
                }
            ...
            else if (d.getName().contains(RoomNumber+DeviceTypes.ServiceSwitch.toString())) {
                Log.d("serviceProb",RoomNumber+" "+d.getName() +" "+RoomNumber+DeviceTypes.ServiceSwitch);
                if (services == null) {
                    services = new ArrayList<>();
                }
                CheckinServiceSwitch ss = new CheckinServiceSwitch(d,this);
                Log.d("serviceProb",RoomNumber+" "+ss.device.name);
                services.add(ss);
                devs.add(ss);
            }
            ...
            else if (d.getName().contains(RoomNumber+DeviceTypes.Switch.toString())) {
                if (switches == null) {
            ...
            else if (d.getName().contains(RoomNumber+DeviceTypes.DoorSensor.toString())) {
                if (doorSensors == null) {
            ...
            else if (d.getName().contains(RoomNumber+DeviceTypes.Curtain.toString())) {
                if (curtains == null) {
            ...
            else if (d.getName().contains(RoomNumber+DeviceTypes.AC.toString())) {
                if (acs == null) {
                    acs = new ArrayList<>();
                }
                ...
                public void close() {
                    ...
                    if (RoomNumber == 110 || RoomNumber == 109) {
                        ...
                        t.schedule(new TimerTask() {
                            @Override
                            public void run() {
                                Log.d("powerScen" , RoomNumber+" time finish "+powerScene);
                                if (powerScene) {
            ...
            for (CheckinMotionSensor ms : motionSensors) {
                ms.listen(new MotionListener() {
                    ...
                    public void motionDetected() {
                        Log.d("powerScen" , RoomNumber+" motion detected");
                        Tuya.LastListenersActionTime = Calendar.getInstance(Locale.getDefault()).getTimeInMillis();
                        if (RoomNumber == 110 || RoomNumber == 109) {
    ...
    public CheckinServiceSwitch getMainServiceSwitch() {
        if (isHasServiceSwitch()) {
            for (CheckinServiceSwitch ss :services) {
                Log.d("serviceProb",RoomNumber+" service main "+ss.device.getName());
            }
    ...
    public void laundryRoomOff(IResultCallback result) {
        if (services == null || services.isEmpty()) {
            ...
            services.get(0).laundryOff(new IResultCallback() {
                @Override
                public void onError(String code, String error) {
                    result.onError(code,error);
                }
...
//    }
    ...
    public void setClientIn(int status) {
        Log.d("powerScen" , "guest "+status);
        fireRoom.child("ClientIn").setValue(status);
    }
