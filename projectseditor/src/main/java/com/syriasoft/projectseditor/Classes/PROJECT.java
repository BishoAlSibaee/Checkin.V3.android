...
import com.syriasoft.projectseditor.Interfaces.ProjectLoginCallback;
...
public class PROJECT {
    ...
    public List<ControlDevice> myControlDevices;
    public PROJECT_VARIABLES myProjectVariables;
    ...
    public PROJECT(int id, String projectName, String city, String salesman, String tuyaUser, String tuyaPassword, String lockUser, String lockPassword, String url) {
        ...
        this.url = url;
        myControlDevices = new ArrayList<>();
    }
    ...
    public static void getProjects(RequestQueue queue, String url, GetProjectsCallback callback) {
        queue.add(new StringRequest(Request.Method.POST, url, response -> {
            try {
                List<PROJECT> projects = new ArrayList<>();
                JSONArray arr = new JSONArray(response);
                for (int i=0;i<arr.length();i++) {
