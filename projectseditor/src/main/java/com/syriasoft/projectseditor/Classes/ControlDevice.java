...
import com.syriasoft.projectseditor.Classes.Property.PropertyDB;
import com.syriasoft.projectseditor.Classes.Property.Room;
import com.syriasoft.projectseditor.Interfaces.ControlDeviceCallback;
import com.syriasoft.projectseditor.Interfaces.ControlDeviceListener;
import com.syriasoft.projectseditor.Interfaces.GerRoomsCallback;
import com.syriasoft.projectseditor.Interfaces.GetControlDevicesCallback;
import com.syriasoft.projectseditor.Interfaces.GetDeviceLastWorkingTime;
...
public class ControlDevice {
    ...
    public DatabaseReference deviceReference;

    public long lastWorking;
    ...
    public void setMyFBReference(DatabaseReference ref) {
        deviceReference = ref;
    }
    ...
    public static void addNewControlDevice(RequestQueue queue,PROJECT project, ControlDeviceCallback result) {
        queue.add(new StringRequest(Request.Method.GET, project.url + addNewControlDevice, response -> {
            try {
    ...
    public void deleteControlDevice(RequestQueue Q, PROJECT project, RequestCallback callback) {
        String url = project.url + deleteControlDevice;
    ...
    public static void getControlDevices(RequestQueue queue,PROJECT project, GetControlDevicesCallback result) {
        queue.add(new StringRequest(Request.Method.GET, project.url+"roomsManagement/getServerDevices", response -> {
            try {
                List<ControlDevice> devices = new ArrayList<>();
                JSONArray arr = new JSONArray(response);
                for (int i=0;i<arr.length();i++) {
        ...
        if (pDB.isRoomsInserted()) {
            ...
            setControlDeviceFirebaseListener(deviceReference, new ControlDeviceListener() {
                ...
                public void onRoomsChanged() {
                    queue.add(new StringRequest(Request.Method.POST,myProject.url+getRoomsUrl, response -> {
